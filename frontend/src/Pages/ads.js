...
function Window(){
    ...
    const handleMouseEnterArtist = () => {
        setIsEnteredArtist(true)
    }
    const handleMouseLeaveArtist = () => {
        setIsEnteredArtist(false)
    }
    const handleMouseDownVenue = () => {
        setIsClickedVenue(!isClickedVenue)
    }
    const handleMouseDownArtist = () => {
        setIsClickedArtist(!isClickedArtist)
    }
    const handleMouseEnterVenue = () => {
        setIsEnteredVenue(true)
    }
    const handleMouseLeaveVenue = () => {
        setIsEnteredVenue(false)
    }
    ...
    if(advertisements.length && isLoading){
        setIsLoading(false)
    }
    ...
    return (
        <div>
            <div className={"filter"}>
                <div className={"filter_box text_description_regular margin_left"}>
                ...
                <div className={isClickedArtist ? "filter_box_clicked text_description_regular" : (isEnteredArtist ? "filter_box_entered text_description_regular" : "filter_box text_description_regular") } onMouseEnter={handleMouseEnterArtist} onMouseLeave={handleMouseLeaveArtist} onClick={handleMouseDownArtist}>Исполнители</div>
                <div className={isClickedVenue ? "filter_box_clicked text_description_regular" : (isEnteredVenue ? "filter_box_entered text_description_regular" : "filter_box text_description_regular") } onMouseEnter={handleMouseEnterVenue} onMouseLeave={handleMouseLeaveVenue} onClick={handleMouseDownVenue}>Площадки</div>
            </div>
            <div className={"title"}>
                <p className={"title_text"}>Объявления исполнителей и площадок</p>
            </div>
            {isLoading ? (
                <div className="load"/>
            ) : (<>
            <div className={"parent-container"}>
                <div className={"wrapper"}>
                    {advertisements.map((item, index) => (
                        <div
                            key={item["ad_id"]}
                            className={isClickedArtist || isClickedVenue ? (item["type"] === "venue_ad" ? (isClickedVenue ? (isEntered[index] ? "background_hover" : "background") : "background_hidden") : (isClickedArtist ? (isEntered[index] ? "background_hover" : "background") : "background_hidden")) : (isEntered[index] ? "background_hover" : "background")}
